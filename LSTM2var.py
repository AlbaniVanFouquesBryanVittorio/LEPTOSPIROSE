...
"""
#Modele LSTM prenant deux jeux de données de meme taille (qte pluie et qte lepto) 
import numpy as np
...
# load the dataset
dataframe = read_csv("test2.csv", usecols=[2], engine='python')
dataframe2 = read_csv("test2.csv", usecols=[3], engine='python')


df1 = dataframe.values
df1 = df1.astype('float32')

df2 = dataframe2.values
...
train_output = int(len(df2) * 0.50)
test_output = int(len(df2) * 0.30)
validation_output = int( len(df2) * 0.20)

trainOutput,validationOutput ,testOutput = df2[0:train_output,:],df2[train_output:train_output+validation_output,:] ,df2[train_output+validation_output:len(df2),:]
...
#for i in range(20):
model.fit(trainX, trainY, validation_data=(validationX,validationY), epochs=epochs, batch_size=batch_size, verbose=2, shuffle=False)
model.reset_states()
...
#ValidScore = math.sqrt(mean_squared_error(validationY[0], trainPredict[:,0]))
...
testPredictPlot[(len(trainPredict)+len(validationPredict))+(look_back*3):len(df1), :] = testPredict
